...
__all__ = [
    "pvt_tiny_256", "pvt_small_256", "pvt_medium_256", "pvt_large_256"
]
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if i != self.num_stages - 1:
                ...
                outputs[f"res{i+2}"] = x[:, 1:].reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
...
@register_model
def pvt_tiny_256(pretrained=False, **kwargs):
...
@register_model
def pvt_small_256(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        img_size=256, patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
...
@register_model
def pvt_medium_256(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        img_size=256, patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
...
@register_model
def pvt_large_256(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        img_size=256, patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
...
@register_model
def pvt_huge_v2_256(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        img_size=256, patch_size=4, embed_dims=[128, 256, 512, 768], num_heads=[2, 4, 8, 12], mlp_ratios=[8, 8, 4, 4],
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 10, 60, 3], sr_ratios=[8, 4, 2, 1],
